...
This pipeline generates candidate TAD regions based on CTCF and cohesin (currently only RAD21) ChIP.
...
def makeInsulators(infiles, outfile):
    ...
    insulator_dist = PARAMS["insulator_peaks_merge_dist"]
    ...
    if len(rad21)==0:
        ...
        statement = '''cat %(ctcf)s | 
                         sort -k1,1 -k2,2n - |
                         mergeBed -c 4,5 -o collapse,mean -d %(insulator_dist)s -i - |
    ...
    if len(rad21)==1:
        ...
        statement = '''ctcf=`mktemp -p %(tmp_dir)s`; checkpoint;
                       ...
                       cat $ctcf <( cut -f1-5 %(rad21)s ) |
                         sort -k1,1 -k2,2n - |
                         mergeBed -c 4,5 -o collapse,mean -d %(insulator_dist)s -i - > %(outfile)s'''
