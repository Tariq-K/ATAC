...
Pipeline for the analysis of ATAC-seq data
...
Default configuration files can be generated by executing:
   ...
   python <srcdir>/pipeline_atac.py config

Input files
-----------
- unmapped but QC'd and trimmed fastq files
- sample files should be of format: (sample_name)_(replicate).fastq.*.gz
  e.g. sample1_r1.fastq.1.gz
  * replicate no. must be in format: "r1", not "1" 
...
def macs2Predictd(infile, outfile):
    ...
    venv = PARAMS["macs2_venv"]
    
    statement = '''%(venv)s; checkpoint;
                     macs2 predictd 
                     --format BAM 
                     --ifile %(infile)s 
                     --outdir %(outdir)s 
...
def mergeReplicatePeaksGenerator():
    '''Group samples into replicates for merging'''

    if PARAMS["replicates_merge_reps"]=="False":
        peaksets = []
    else:
        peaksets = [".all", ".size_filt"]

    if bool(len(peaksets)):
...
def hmmratac(infile, outfile):
    ...
    statement = '''blacklist=`mktemp -p $SCRATCH_DIR`; checkpoint;
                   zcat %(blacklists)s > $blacklist; checkpoint;
                   java -Xms20G -Xmx40G -jar %(exe)s 
                     ...
                     -o %(name)s
                     %(options)s
                     --blacklist $blacklist; checkpoint;
                   rm $blacklist'''
...
def FRIP(infile, outfile):
        ...
        # bowtie2.dir/Neu_SP_1.size_filt.prep.bam
        # Neu_SP_1_fripcounts.txt
...
# @transform("bowtie2.dir/*.prep.bam",
#            regex(r"bowtie2.dir/(.*)_r[1-2].prep.bam"),
#            r"deeptools.dir/\1.nucleosome.coverage.bw")
@follows(indexPrepBam)
@active_if(Unpaired == False)
...
def bamCoverage_mononuc(infile, outfile):
    ...
    #infile = ''.join([x for x in [infile] if "size_filt" not in x])
